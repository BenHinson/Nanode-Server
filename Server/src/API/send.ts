...
const Read_Send_Contents = async(NodeData:ReadSendData, Connection:Response) => {
  ...
  let result = await Node.Read({userId, type, section, subSection, "nodeIds": path, contents});
  ...
  if (result != 'Empty Query') {
    ...
    for (const [id, data] of Object.entries(result)) {
      if (Format == 'bin') {
        ...
        if (!data.type.file) {
          fileTree = await Node.Read({"userId": NodeData.userId, "type": 'TREE', "section": NodeData.section, "nodeIds": [id], "contents": false});
        }
        let itemParent = await Node.Read({"userId": NodeData.userId, "type": 'SPECIFIC', 'section': data.BIN_DATA.section, "nodeIds": [data.BIN_DATA.parent], "keys": ["name", "description"]});
