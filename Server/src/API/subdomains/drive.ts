...
const corsOptions = {
  origin: ['https://drive.nanode.one', 'http://localhost:3000'],
};
...
Drive_Router.use((req, res, next) =>
  req.get('content-type')?.indexOf('multipart/form-data') !== 0
    ? next()
...
Drive_Router.get('/', Nauth.Middle, async (req, res) => {
  res.sendFile('F://Nanode/Nanode Client/views/drive.html', {
    extensions: ['html', 'htm'],
  });
});
...
Drive_Router.use('/storage/:content', Nauth.Middle, async (req, res, next) => {
  ...
  if (!Nelp.validateClient('nodeId', wantedURL)) {
    return Nelp.errorPage(res);
  }
  let imgHeight =
    req.query.h == 'null' ? null : parseInt((req.query.h as any) || null);
  let imgWidth =
    req.query.w == 'null' ? null : parseInt((req.query.w as any) || null);
  let section = Nelp.validateClient('section', req.query.s as Sections)
    ? (req.query.s as Sections)
    : 'main';
  ...
  if (nodeData && nodeData?.type?.file) {
    if (!imgHeight && !imgWidth) {
      Recent.Push({userId, section, nodeId: wantedURL});
    }

    let resize =
      nodeData.type.mime.includes('image') &&
      !nodeData.type.mime.includes('svg') &&
      (imgWidth || imgHeight)
        ? {width: imgWidth || undefined, height: imgHeight || undefined}
        : false;

    let thumbnail =
      nodeData.contents?.thumbnail &&
      resize &&
      resize.height === 90 &&
      resize.width === 120
        ? nodeData.contents?.thumbnail
    ...
    ReadWrite.Mass(res, {
      fileId: nodeData.contents.file,
      userId,
      nodeId: wantedURL,
      section: section,
      mimetype: nodeData.type.mime,
      resize: resize,
      thumbnail: thumbnail,
    });
  } else {
...
Drive_Router.use(
  ...
  Nauth.Middle,
  async (req, res, next) => {
    const section = Nelp.validateClient('section', req.params.section)
      ? (req.params.section as Sections)
      : 'main';
    const item = Nelp.validateClient('nodeId', req.params.item)
      ? req.params.item
      : undefined;
    ...
    if (userId && section && item) {
      ...
      if (node[item]) {
        if (section == 'bin') {
          node = await Send.FormatResponse('binInfo', node, {
            userId,
            section,
          });
        } else {
...
Drive_Router.use('/folder/:nodeId', Nauth.Middle, async (req, res, next) => {
  ...
  let section = Nelp.validateClient('section', req.query.s as Sections)
    ? (req.query.s as Sections)
    : 'main';
  let subSection = Nelp.validateClient(
    'subSection',
    req.query.sub as SubSections,
  )
    ? (req.query.sub as SubSections)
    : undefined;
...
Drive_Router.use(
  ...
  async (req, res, next) => {
    let userId = req.headers.userId as UserId;
    let section = Nelp.validateClient('section', req.params.section)
      ? (req.params.section as Sections)
      : 'main';
...
Drive_Router.use('/account/:data?', Nauth.Middle, async (req, res, next) => {
  ...
  if (req.params.data == 'bin') {
    ...
    if (
      typeof accountData[0].settings == 'undefined' ||
      accountData[0].settings == false
    ) {
      await Account.Write({
        userId,
        type: 'SET',
    ...
    return res.send({
      ...accountData[0].settings,
      ...{
        plan: {max: accountData[0].plan.max, used: accountData[0].plan.used},
      },
    });
  }
});
...
Drive_Router.post('/bin', Nauth.Middle, async (req, res) => {
  ...
  if (!Nelp.validateUUID(nodeId)) {
    return Send.Message(res, 400, {message: 'Invalid Item ID'});
  }
  ...
  try {
    ...
    if (action == 'RESTORE') {
      ...
      let filesToDelete = await Node.Edit({
        userId,
        type: 'DELETE',
        section: 'bin',
        subSection,
    ...
    console.log(err);
    return Send.Error({Message: `Failed to ${action}`, Code: 400}, res);
  }
});
...
Drive_Router.post('/search', Nauth.Middle, async (req, res) => {
  ...
  let searchResults = await Search.Find({
    userId,
    input: body.input,
    params: body,
  });
...
Drive_Router.post('/create', Nauth.Middle, async (req, res) => {
  ...
  const section = Nelp.validateClient('section', req.body.section)
    ? req.body.section
    : 'main';

  if (parent == 'SEARCH')
    return Send.Error(
      {Message: 'Cannot Create a Folder within Search', Code: 405},
      res,
    );
  if (parent == '_MAIN_') parent = '_GENERAL_';

  if (type.match(/Item|Folder|File|Span/i)) {
    const written = await Node.Create(
      type,
    ...
    if (written) {
      return await Send.Read_Send_Contents(
        {userId, type: 'ID', section, path: [path], contents: false},
        res,
      );
    } else {
...
Drive_Router.post('/edit', Nauth.Middle, async (req, res) => {
  ...
  if (!Nelp.validateUUID(editItemIDs[0])) {
    return Send.Message(res, 400, {message: 'Invalid Item ID'});
  }
  ...
  for (let i = 0; i < editItemIDs.length; i++) {
    // Iterate Through All Edited Items. If a bad write, break loop and send error.
    Edit.nodeId = editItemIDs[i];
    let write = await Node.Edit(Edit);
    if (!write || write.error) {
      return await Send.Error(
        {
          Message: write.error.message || 'Internal',
          Code: write.error.code || 400,
        },
        res,
      );
    }
  ...
  if (action !== 'BIN') {
    Recent.Push({userId, section, nodeId: editItemIDs});
  }

  if (path) {
    // Path states that we send the directory back to the User
    return await Send.Read_Send_Contents(
      {
        userId,
        type: 'ID',
...
Drive_Router.post('/share', Nauth.Middle, async (req, res) => {
  ...
  if (action && nodeId && section && userId) {
    if (action == 'LINK') {
      let linkId,
        fileName = uuidv3(nodeId, userId);

      let calledInformation = await Node.Read({
        userId,
        type: 'SPECIFIC',
        section,
        nodeIds: [nodeId],
      ...
      if (calledInformation[nodeId]?.share?.link) {
        ...
        linkId = await Links.writeShareLink(nanoid(16), userId, {
          nodeId,
          fileName,
          mime: calledInformation[nodeId].type.mime,
        });
        await Node.Edit({
          userId,
          type: 'DATA',
          section,
          nodeId,
          changeTo: {share: {link: {url: linkId}}},
        });
      }
...
Drive_Router.post('/download', Nauth.Middle, async (req, res) => {
  ...
  if (forUser && name && items && section) {
    try {
      return await Modify.ZipFile(res, req.headers.userId as UserId, req.body);
    } catch (error) {
      console.error(error);
    }
  }
...
Drive_Router.post('/auth', Nauth.Middle, async (req, res) => {
  ...
  let access = await Security.Checker({
    userId,
    section,
    nodeId,
    wanted: 'Access',
...
Drive_Router.post('/upload', Nauth.Middle, async (req, res) => {
  ...
  if (message === 'Queue_Empty') {
    ReadWrite.UploadCheck(userId, true);
    return res.sendStatus(200);
  }

  if (message === 'Cancelled') {
    console.log('Upload Cancelled, empty the Tree and Remove file chunks?');
    return res.sendStatus(200);
  }

  if (meta?.parent === 'SEARCH' || !meta?.parent) {
    return Send.Message(res, 405, {
      status: 'Invalid',
      message: 'Not a valid upload destination',
    });
  }
  ...
  let allocation = await Security.Upload_Limit(
    userId,
    uploadChunkSize,
    chunkInfo,
    meta,
  ); // Checks User Plan against the upload.
  ...
  if (allocation.auth === true && fileData) {
    ...
    if (result.chunkWrite) {
      ...
      // Entire File written, add to Upload_Tree and request next file.
      meta.size = allocation.size as number;
      meta.type = result.fileType?.mime || meta.type;
      await ReadWrite.Write_To_User_File(
        userId,
        result.fileNodeId as string,
        meta,
      );
      return Send.Message(res, 200, {
        status: 'Complete',
  ...
  } else if (allocation.auth === false) {
    // The user does not have enough allocated space to upload the content.
