...
const Mass = function (res: Response, readData: ReadData): Buffer | false {
  const fileName = readData.thumbnail
    ? readData.thumbnail
    : readData.fileId || uuidv3(readData.nodeId, readData.userId);
  const fileDirectory = readData.thumbnail
    ? ThumbnailDirectory + fileName
    : MassDirectory + fileName;
...
const WriteThumbnail = async function (
  fileName: string,
  data: Buffer,
): Promise<Boolean> {
  await fse.promises
    .writeFile(ThumbnailDirectory + fileName, data)
    .then(() => {
...
const Upload = async function (chunk: Chunk): Promise<UploadReturn> {
  ...
  const PreviousChunkName =
    ChunkStorage + ['CHUNK', nodeId, index - 1].join('-');

  if (index > 0) {
    // Add to Previous
    await fse.promises.appendFile(PreviousChunkName, fileArray).catch(err => {
      console.log(err);
      return 'Failed';
    });
    await fse.promises.rename(PreviousChunkName, ThisChunkName).catch(err => {
      console.log(err);
      return 'Failed';
    });
  } else {
    await fse.promises.writeFile(ThisChunkName, fileArray).catch(err => {
      console.log(err);
      return 'Failed';
    });
  }
  ...
  if (index >= total - 1) {
    // Finish Up by Moving File and Renaming
    let fileNodeId = uuidv1();
    let fileType = await FileType.fromFile(ThisChunkName);
    await fse.promises
      .rename(ThisChunkName, EndStorage + uuidv3(fileNodeId, userId))
      .catch(err => {
        console.log(err);
        return 'Failed';
      });
...
const Write_To_User_File = async function (
  userId: UserId,
  nodeId: string,
  meta: UploadMeta,
) {
  if (!uploadObjectTree[userId]) {
    uploadObjectTree[userId] = [];
  }
...
const Create_Folders = async (
  ...
  for (let i = 0; i < relativePath.length; i++) {
    ...
    let found = uploadObjectTree[userId].find(
      (item: any) => item[relativePath[i]],
    );
    if (found) {
      previousFolder = found[relativePath[i]].id;
      continue;
    }
    let fID = uuidv1();
    uploadObjectTree[userId].push({[relativePath[i]]: {id: fID}});
    await Create_New_Item(userId, fID, previousFolder ?? meta.parent, {
      ...meta,
      ...{section: meta.section, name: relativePath[i]},
    });
    previousFolder = fID;
  }
...
const Create_New_Item = async (
  ...
  let writtenSize: number = await Node.Create(
    'Item',
