...
const Login = async (
  ...
  Account_Coll.find({email: email.toLowerCase()}, {$exists: true}).toArray(
...
const Create = async (
  ...
  let check = await Account_Coll.find({email}, {$exists: true})
    .limit(1)
    .toArray();
  if (check.length) {
    console.log('Account Exists Already');
    return res.send({Acc_Server: 'Already_Exists'});
  }
  ...
  return Account_Coll.insertOne({
    ...
    plan: {max: 10 * 1024 * 1024 * 1024, used: 0},
  })
    .then(async () => {
...
const Write = async (params: WriteParams) => {
  ...
  if (type == 'INCREMENT') {
    mongoEdit.$inc = {[`${parentKey}.${childKey}`]: data || 1};
  } else if (type == 'SET') {
    mongoEdit.$set = {[`${parentKey}.${childKey ? childKey : ''}`]: data};
  } else if (type == 'UNSET') {
    mongoEdit.$unset = {};
    Array.isArray(childKey)
      ? childKey.forEach(
          item => (mongoEdit.$unset[`${parentKey}.${item ? item : ''}`] = ''),
        )
  ...
  return Account_Coll.updateOne({userId}, mongoEdit)
    ...
    .catch((err: any) => {
      console.error(`Couldn't update account info:  ${err}`);
      return false;
    });
};
