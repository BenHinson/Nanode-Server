...
// Node.ts > NODES
interface AccountBaseNodes {
  // Account_Data. VIA: Account_Setup
  _id: string;
...
interface Node extends SecurityValue {
  ...
  contents: {
    // shh dont tell typescript this can be ShortNode[] | SpanNode[]
    file: string;
    drive: string | 'F';
  };
  type: {
...
type MongoObject = {
  [key in '$pull' | '$set' | '$unset' | '$push' | '$inc' | string]: any;
};
