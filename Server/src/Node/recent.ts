...
const Push = async (DATA: {
  ...
  let currentRecentNodes = await Node.Custom_Read({
    userId,
    query: {[`recent.${section}`]: 1},
  });
  if (!currentRecentNodes['recent'][section]) {
    return false;
  }
...
const Fetch = async (DATA: {userId: UserId; section: Sections}) => {
  // https://drive.nanode.one/activity/recent/main
  const {userId, section} = DATA;

  let recentIds = await Node.Custom_Read({
    userId,
    query: {[`recent.${section}`]: 1},
  });
