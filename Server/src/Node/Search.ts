...
const Match = async (
  ...
  let result = [],
    searchEnd = null;
  let userNodes: GroupNodes = await LoadNodes(
    userId,
    search.section,
    search.withinParent,
  ); // IMPORTANT: if loading from withinParent, check if its locked first. locked would require key. hmmm.

  if (from !== 0) {
    userNodes = Trim(userNodes, from);
  }
  ...
  for (const [id, data] of Object.entries(userNodes)) {
    ...
    if (!search.forFiles && data.type.file == true) {
      continue;
    }
    if (!search.forFolders && data.type.file == false) {
      continue;
    }
    if (search.onlyShared && !data.share) {
      continue;
    }
    ...
    // Search For:
    if (
      (search.color && data.color == search.color) ||
      ...
      // Uses modified atm, could add created / deleted later.
      let modifiedDate = new Date(data.time?.modified?.stamp);
      if (
        modifiedDate < new Date(search.date.max) &&
        modifiedDate > new Date(search.date.min)
      ) {
        result.push(SafeForWeb(userNodes[id]));
        continue;
      }
    ...
    if (
      search.description &&
      data?.description?.toLowerCase().includes(search.input.toLowerCase())
    ) {
      result.push(SafeForWeb(userNodes[id]));
      continue;
    }
    if (search.prevNames && data?.previous?.includes(search.input)) {
      result.push(SafeForWeb(userNodes[id]));
      continue;
    }
  }
...
const Options = (
  ...
  if (color) {
    search.color = input;
  } // Can be true with a name input; IE: 'red' In name or as colour?  Could set input to color and use input for search
  if (type) {
    search.type = input;
  }
  if (date) {
    search.date = {max: input, min: inputTwo || input};
    delete search.name;
  } // Search for Date between two inputs.   Cannot have Name
  if (size) {
    search.size = {max: input, min: inputTwo || input};
    delete search.name;
  } // Search For size between two inputs.   Cannot have Name

  if (forFolders) {
    search.forFolders = true;
  } // Ignore anything that isn't the following if true.
  if (forFiles || (!forFiles && !forFolders)) {
    search.forFiles = true;
  }
  if (onlyShared) {
    search.onlyShared = true;
  }

  if (withinParent && Nelp.validateUUID(withinParent)) {
    search.withinParent = withinParent;
  } // withinParent given as withinParent. input && inputTwo saved for the search values. Parent can be a span ID

  if (description) {
    search.description = true;
  } // Also checks descriptions and preNames if there are.
  if (prevNames) {
    search.prevNames = true;
  }
...
const Trim = (nodes: GroupNodes | any, from: number) => {
  // 'Load More' has been Called. Remove Already Found Objects.
...
const Score = (node: Node) => {
  // Provide an ordering score based on params
  // Match - Max Score
  // Includes - Less Score
};

const SafeForWeb = (node: Node) => {
  // Remove any compromising data from the node
  return {
    id: node.id,
    name: node.name,
    parent: node.parent,
