...
const Nelp = {
  ...
  validateClient: (variable:'section'|'subSection'|'nodeId', input:string='') => {
  ...
  dupeNameIncrement: (parentObject:object | any, name:string='', num:number=0): string => {
    return typeof parentObject[num+"_"+name] == 'undefined' ? (num == 0 ? name : num+"_"+name) : Nelp.dupeNameIncrement(parentObject, name, num+1);
  },
  ...
  deviceMatch: (thisDevice:object, registered:any): boolean => {
    ...
    for (const [key, value] of Object.entries(thisDevice)) {
      if (registered[key] != value) {deviceDifference++;}
    }
